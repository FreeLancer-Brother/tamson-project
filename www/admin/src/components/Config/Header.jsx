...
const HeaderConfig = () => {
  ...
  useEffect(() => {
    if (saving && activeKey === "4") {
      form
        .validateFields()
        .then(async (values) => {
          const { facebookLink, youtubeLink ,instagramLink} = values;
          ...
          try {
            const response = await agent.put(`${api.configs}/header`, {
              ...postData,
            });
            if (_.get(response, "data.success")) {
              message.success("Update header successfully");
            }
          } catch (error) {
  ...
  useEffect(() => {
    async function getData() {
      const response = await agent.get(`${api.configs}/header`);
      ...
      setImage(([convertImageToObject(_.get(data, "logo"))]));
    }
    try {
      getData();
    } catch (error) {
